...
import { getFormById, updateForm, Form, FileAttachment, ContentBlock, Standard, getStandards } from '@/lib/data';
    ...
    if (id) {
      const fetchForm = async () => {
        ...
        if (formData) {
          ...
          const loadedDate = formData.issue_date ? new Date(formData.issue_date + 'T00:00:00Z') : undefined;
          setIssueDate(loadedDate);
          setPageCount(formData.page_count ?? '');
          setFiles(formData.files || []);
          setBlocks(formData.content_blocks || []);
          setSelectedStandard(formData.standard_id || undefined);
        }
        setIsLoading(false);
      }
      fetchForm();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: Partial<Omit<Form, 'id' | 'last_updated'>> = {
        ...
        // Set old fields to null to clean up DB
        table_data: null,
        paragraph_template: null,
        files,
        standard_id: selectedStandard === 'none' ? null : selectedStandard,
      };
